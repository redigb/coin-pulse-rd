...
import { getCandlestickConfig, getChartConfig, PERIOD_BUTTONS, PERIOD_CONFIG } from "@/consts";
...
export const CandlestickChart = ({ children, data, coinId, height = 360,
    initialPeriod = 'daily',
    ...
    const fetchOHLData = async (selectedPeriod: Period) => {
        try {
            const { days } = PERIOD_CONFIG[selectedPeriod];
            const newData = await fetcher<OHLCData[]>('/coins/bitcoin/ohlc', {
                vs_currency: 'usd',
                days,
                //interval: 'hourly', config.interval
                precision: 'full'
            });
            setOhlcData(newData ?? []);
        } catch (error) {
            ...
            <div className="chart-header">
                ...
                <div className="button-group">
                    ...
                    {PERIOD_BUTTONS.map(({ value, label }) => (
                        <button key={value} className={period === value ?
